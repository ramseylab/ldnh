...
## For a melted data frame "raw_df_fac" of voltammograms for various
## carbamazepine levels, and given various hyperparameter values (window_center,
## window_width, theta, nug, ext_type), compute a "signal" value (according to
## the method specified in the argument "ext_type") from the residuals from a
## Gaussian process fit to the window-censored voltammogram; find the best-fit
## linear model relating carbamazepine to signal values; compute the Pearson
## product-moment coefficient between the signal values and the annotated
## carbamzepine levels, and also back-convert each signal value to an estimated
## carbamazepine level and compute the average normalized error between the
## estimated and actual carbamazepine level, across all carbamazepine levels and
## replicates; return the results as a list. Due to limitations of the
## "expand.grid" function, the extractor type cannot be passed as a string, so
## instead pass a numeric for "ext_type", which is mapped internally to a string
## as follows: 0.0 => "min" and 1.0 => "integral". For theta and nug, to let the
## Gaussian process fit them, just pass values of -1.0 for either (or both) as
## desired; you can choose to have the Gaussian process fit both, or none, or
## either.
...
xlsx::read.xlsx(file_spec, sheetIndex=sheet_index, header=TRUE) %>% 
    within({
    ...
    ##        detilt_data_avg_blanks("rel_log_neg_current") %>%
    ...
    make_curves_kiss_avg("rel_log_neg_current_sub", c(peakfind_start_v, peakfind_end_v)) %>%
